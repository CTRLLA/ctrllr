...
function processAssertions(config) {
    ...
    if (typeof test.expectKeys !== 'undefined') {
        var obj = response.body instanceof Array ? response.body[0] : response.body;

        // TODO: make sure response is object
        // TODO: make sure `expectKeys` is array

        if (!obj) {
            printAssertion(false, 'Failed assertion: expectKeys');
            printAssertionMeta(false, 'EXPECTED: ' + test.expectKeys.toString());
            printAssertionMeta(false, 'RECEIVED: ' + obj);
        } else {
