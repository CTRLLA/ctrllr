...
CTRLLR.prototype.run = function(test, inQueue) {
    ...
    var
        ...
        buildUrl = function() {
            ...
            safeAsync(test.url, alias.options.timeout, [alias.getHelper()])
                .then(function(url) {
                    if (alias.options.server) {
                        deferred.resolve('http://localhost:' + alias.options.port + url);
                    } else {
                        deferred.resolve((alias.options.baseUrl || '') + url);
                    }
                }).fail(function(err) {
                    deferred.reject(err);
                });
        ...
        makeRequest = function() {
            buildUrl().then(function(url) {
                var req = request[(test.method || 'GET').toLowerCase()](url);
                ...
                log.error('Error building url.', err);
                finish();
            });
        },
