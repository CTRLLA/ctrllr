...
function buildUrl(config) {
    ...
    // calls safeAsync with `test.url` being the target value
    // also gives it a max timeout and the ctrllr helper to pass
    // to `test.url` as an argument if it is a function
    var
        timeout = test.timeout || ctrllr.options.timeout,
        args = [ctrllr.getHelper()];

    safeAsync(test.url, timeout, args)
        .then(function(url) {
            if (ctrllr.options.server) {
                // if running express server, prepend local url
                request.url = 'http://localhost:' + ctrllr.options.port + url
                deferred.resolve(config);
            } else {
                request.url = (ctrllr.options.baseUrl || '') + url;
                deferred.resolve(config);
            }
        }).fail(function(err) {
            deferred.reject(err);
...
function after(config) {
    ...
    /*
     ...
     var
     fn = queue[0],
     result = fn(response, ctrllr.getHelper());
...
function makeRequest(config) {
    ...
    safeAsync(test.path || test.url, timeout, args)
        .then(function(url) {
            var fullUrl;
